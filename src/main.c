    ...
    for (int i = 0; i < MAX_SCORE_POPUPS; i++)
    {
        if (!score_popups[i].active)
        {
            score_popups[i].position = (Vector2){
                ...
                origin.y + y * TILE_SIZE + TILE_SIZE / 2};
    ...
    for (int y = 0; y < BOARD_SIZE; y++)
    {
        for (int x = 0; x < BOARD_SIZE - 2; x++)
        {
            ...
            if (t == board[y][x + 1] && t == board[y][x + 2])
            {
                ...
                add_score_popup(x, y, 100, grid_origin);
            }
        }
    }
...
void resolve_matches()
{
    for (int x = 0; x < BOARD_SIZE; x++)
    {
        ...
        // fill the empty spaces with new random tiles
        while (write_y >= 0)
        {
...
int main(void)
{
    ...
    score_font = LoadFontEx("assets/font.ttf", SCORE_FONT_SIZE, NULL, 0);
    ...
    while (!WindowShouldClose())
    {
        ...
        {
            int x = (int)((mouse.x - grid_origin.x) / TILE_SIZE);
            int y = (int)((mouse.y - grid_origin.y) / TILE_SIZE);
            ...
            if (x >= 0 && x < BOARD_SIZE && y >= 0 && y < BOARD_SIZE)
            {
                ...
                else
                {
                    if (are_tiles_adjacent(selected_tile, current_tile))
                    {
                        ...
                        selected_tile = (Vector2){-1, -1}; // Deselect after swap
                    }
                    else
                    {
                        selected_tile = current_tile; // Select new tile
                    }
                }
            }
        }
            ...
            for (int x = 0; x < BOARD_SIZE; x++)
            {
                ...
                if (board[y][x] != ' ')
                {
                    DrawTextEx(
                        ...
                        (Vector2){
                            rect.x + TILE_SIZE / 4,
                            rect.y + TILE_SIZE / 4 - fall_offset[y][x]},
                        20,
                        1,
        ...
        for (int i = 0; i < MAX_SCORE_POPUPS; i++)
        {
            if (score_popups[i].active)
            {
                DrawTextEx(
                    ...
                    (Vector2){
                        score_popups[i].position.x - TILE_SIZE / 4,
                        score_popups[i].position.y - TILE_SIZE / 4},
                    SCORE_FONT_SIZE,
                    1.0f,
